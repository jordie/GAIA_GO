...
func TestRecordStatusChange(t *testing.T) {
	...
	db.Table("appeal_status_changes").
		Where("appeal_id = ? AND new_status = ?", 1, StatusReviewing).
		Count(&count)
...
func TestGetAppealTimeline(t *testing.T) {
	...
	hs.RecordStatusChange(context.Background(), 2, StatusPending, StatusReviewing, "admin", "Started", nil)
	time.Sleep(10 * time.Millisecond)
	hs.RecordStatusChange(context.Background(), 2, StatusReviewing, StatusApproved, "admin", "Approved", nil)
...
func TestGetStatusChangeHistory(t *testing.T) {
	...
	hs.RecordStatusChange(context.Background(), 3, StatusPending, StatusReviewing, "admin", "Review", nil)
	hs.RecordStatusChange(context.Background(), 3, StatusReviewing, StatusApproved, "admin", "Approved", nil)
	...
	if changes[0].NewStatus != StatusReviewing {
		t.Errorf("Expected first change to StatusReviewing, got %s", changes[0].NewStatus)
	}

	if changes[1].NewStatus != StatusApproved {
		t.Errorf("Expected second change to StatusApproved, got %s", changes[1].NewStatus)
	}
}
...
func TestGetStatusDistribution(t *testing.T) {
	...
	statuses := []AppealStatus{StatusPending, StatusReviewing, StatusApproved, StatusPending}
	...
	if dist[StatusPending] != 2 {
		t.Errorf("Expected 2 pending, got %d", dist[StatusPending])
	}

	if dist[StatusApproved] != 1 {
		t.Errorf("Expected 1 approved, got %d", dist[StatusApproved])
	}
}
...
func TestResolutionDaysCalculation(t *testing.T) {
	...
	// Record changes
	hs.RecordStatusChange(context.Background(), 400, StatusPending, StatusReviewing, "admin", "", nil)
	time.Sleep(100 * time.Millisecond)
	hs.RecordStatusChange(context.Background(), 400, StatusReviewing, StatusApproved, "admin", "", nil)
...
func TestTimelineEventOrdering(t *testing.T) {
	...
	for i, status := range []AppealStatus{StatusPending, StatusReviewing, StatusApproved} {
		if i > 0 {
			hs.RecordStatusChange(
				context.Background(),
				401,
				[]AppealStatus{StatusPending, StatusReviewing}[i-1],
				status,
				"admin",
				"",
				nil,
...
func BenchmarkGetAppealTimeline(b *testing.B) {
	...
	appeal := &Appeal{ID: 500, UserID: 700, Status: StatusPending, CreatedAt: time.Now()}
	db.Table("appeals").Create(appeal)

	for i := 0; i < 10; i++ {
		hs.RecordStatusChange(context.Background(), 500, StatusPending, StatusReviewing, "admin", "", nil)
	}
