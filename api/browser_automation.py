    ...
    try:
        ...
        for task in completed + failed:
            if task.recovery_attempts > 0:
                ...
                if task.recovery_succeeded:
                    recovery_rate += 1
        recovery_rate = (
            (recovery_rate / recovery_total * 100) if recovery_total > 0 else 0.0
        )
        ...
        while current_date <= date_to:
            ...
            day_tasks = (
                ...
                .filter(
                    func.date(BrowserTask.completed_at) == current_date,
                    BrowserTask.status.in_(
                        [BrowserTaskStatus.COMPLETED, BrowserTaskStatus.FAILED]
                    ),
                )
                .all()
            )
            ...
            if day_tasks:
                day_avg_time = sum(t.total_time_seconds for t in day_tasks) / len(
                    day_tasks
                )
                day_avg_cost = sum(t.total_cost for t in day_tasks) / len(day_tasks)
                day_cache_hits = sum(
                    1 for t in day_tasks if t.cached_path_used
                ) / len(day_tasks) * 100
                day_recovery_attempts = sum(
                    1 for t in day_tasks if t.recovery_attempts > 0
                )
                day_recovery_success = sum(
                    1
                    for t in day_tasks
                    if t.recovery_attempts > 0 and t.recovery_succeeded
                )
                day_recovery_rate = (
        ...
        logs = (
            db_session.query(
                BrowserExecutionLog.ai_used, func.count(BrowserExecutionLog.id),
